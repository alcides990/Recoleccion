...
public class NumeradorAutoimpresorService {
    ...
    public int consultarSiguiente(DetalleTimbrado detalle, Integer codigoTipoComprobante) {
        ...
        Integer ultimo = jdbc.queryForObject("""
            SELECT COALESCE(MAX(ultimo_numero), 0)
        ...
        return siguienteDentroDelRango(detalle, ultimo, maximoRegistrado(detalle, codigoTipoComprobante));
    }

    private Integer maximoRegistrado(DetalleTimbrado detalle, Integer codigoTipoComprobante) {
        var id = detalle.getDetalleTimbradoPK();
        int serie = detalle.getSerie() == null ? 0 : detalle.getSerie().getCodigoSerie();
    ...
    private int siguienteDentroDelRango(DetalleTimbrado detalle, Integer ultimo, Integer maximo) {
        ...
        long siguiente = Math.max(desde, Math.max(ultimo == null ? 0L : ultimo.longValue(),
                maximo == null ? 0L : maximo.longValue()) + 1);
    ...
    public int reservar(DetalleTimbrado detalle, Integer codigoTipoComprobante) {
        ...
        Integer maximoExistente = maximoRegistrado(detalle, codigoTipoComprobante);
        ...
        Integer ultimo = jdbc.queryForObject("""
            ...
            """, Integer.class, id.getCodigoTimbrado(), id.getCodigoPuntoExpedicion(),
                id.getCodigoSucursal(), codigoTipoComprobante, serie);
        int siguiente = siguienteDentroDelRango(detalle, ultimo, maximoExistente);
        jdbc.update("""
