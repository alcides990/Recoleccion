...
public class AuditoriaComprobanteService {
    ...
    public String capturarFoto(ComprobantePK clave) {
        return jdbc.queryForObject("""
            SELECT JSON_OBJECT(
                       ...
                       'categoria', c.codigo_categoria,
                       'condicionVenta', c.codigo_condicion_venta,
                       'cobrador', c.codigo_cobrador,
                       'cantidadDeuda', c.cantidad_deuda,
                       'cantidadPago', c.cantidad_pago,
                       'tarifa', c.tarifa,
                       'recargo', c.recargo,
                       'saldo', c.saldo,
                       ...
                       'pagos', COALESCE((
                           SELECT JSON_ARRAYAGG(JSON_OBJECT(
                                      'codigoMetodoPago', dp.codigo_metodo_pago,
                                      'importe', dp.importe))
                             FROM detalle_pago dp
                            WHERE dp.numero_comprobante = c.numero_comprobante
                              ...
                              AND dp.codigo_serie = c.codigo_serie), JSON_ARRAY()))
              FROM comprobantes c
             WHERE c.numero_comprobante = ? AND c.codigo_punto_expedicion = ?
               AND c.codigo_sucursal = ? AND c.codigo_tipo_comprobante = ?
               AND c.codigo_serie = ?
            """, String.class, clave.getNumeroComprobante(),
                clave.getPuntoExpedicionPK().getCodigoPuntoExpedicion(),
                clave.getPuntoExpedicionPK().getCodigoSucursal(), clave.getCodigoTipoComprobante(),
                clave.getCodigoSerie());
    }
    ...
    @Transactional(propagation = Propagation.MANDATORY)
    public void registrarModificacion(ComprobantePK clave, Integer usuarioSistema, String motivo,
            String datosAnteriores) {
        jdbc.update("""
            INSERT INTO auditoria_comprobantes
                (accion, numero_comprobante, codigo_punto_expedicion, codigo_sucursal,
                 codigo_tipo_comprobante, codigo_serie, codigo_timbrado,
                 codigo_usuario_sistema, motivo, datos_comprobante, datos_anteriores)
            SELECT 'MODIFICACION', c.numero_comprobante, c.codigo_punto_expedicion,
                   c.codigo_sucursal, c.codigo_tipo_comprobante, c.codigo_serie,
                   c.codigo_timbrado, ?, ?,
                   JSON_OBJECT(
                       'numeroComprobante', c.numero_comprobante,
                       'razonSocial', c.razon_social,
             ...
             WHERE c.numero_comprobante = ? AND c.codigo_punto_expedicion = ?
               AND c.codigo_sucursal = ? AND c.codigo_tipo_comprobante = ?
               AND c.codigo_serie = ?
            """, usuarioSistema, normalizarMotivo(motivo), datosAnteriores,
    ...
    @Transactional(propagation = Propagation.MANDATORY)
    public void renumerarHistorial(ComprobantePK claveAnterior, Integer numeroNuevo) {
        jdbc.update("""
            UPDATE auditoria_comprobantes
             ...
             WHERE numero_comprobante = ? AND codigo_punto_expedicion = ?
               ...
               AND codigo_serie = ?
            """, numeroNuevo, claveAnterior.getNumeroComprobante(),
