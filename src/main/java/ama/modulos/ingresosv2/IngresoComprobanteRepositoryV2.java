...
public interface IngresoComprobanteRepositoryV2 extends JpaRepository<Comprobante, ComprobantePK> {
    ...
    @Query(value = """
            select :serie as serie,
                   case when :escala = 'MES' then month(c.fecha_pago) else day(c.fecha_pago) end as periodo,
                   case when :agrupacion = 'ZONA'
                        then coalesce(nullif(z.zona, ''), 'Sin zona')
                        else coalesce(nullif(trim(concat(coalesce(cob.nombre, ''), ' ', coalesce(cob.apellido, ''))), ''), 'Sin cobrador')
                   end as nombre,
                   sum(c.total_importe) as importe,
                   count(*) as cantidad
            from comprobantes c
            left join cobradores cob on cob.codigo_cobrador = c.codigo_cobrador
            left join servicios s on s.cuenta_corriente = c.cuenta_corriente
              and s.codigo_sucursal = c.codigo_sucursal
            left join manzanas m on m.codigo_manzana = s.codigo_manzana
              and m.codigo_sucursal = s.codigo_sucursal
            left join zonas z on z.codigo_zona = m.codigo_zona
              and z.codigo_sucursal = c.codigo_sucursal
            where c.codigo_sucursal = :sucursal
              and c.codigo_estado = 1
              and c.fecha_pago between :desde and :hasta
            group by case when :escala = 'MES' then month(c.fecha_pago) else day(c.fecha_pago) end,
                     case when :agrupacion = 'ZONA'
                          then coalesce(nullif(z.zona, ''), 'Sin zona')
                          else coalesce(nullif(trim(concat(coalesce(cob.nombre, ''), ' ', coalesce(cob.apellido, ''))), ''), 'Sin cobrador')
                     end
            ...
            """, nativeQuery = true)
    List<IngresoComparativoProjectionV2> comparativo(
            @Param("sucursal") Integer sucursal,
            @Param("serie") String serie,
            @Param("escala") String escala,
            @Param("agrupacion") String agrupacion,
