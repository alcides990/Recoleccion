...
public interface ComprobanteDao extends JpaRepository<Comprobante, ComprobantePK> {
     ...
     @Query(value = """
               SELECT c FROM Comprobante AS c
                ...
                JOIN FETCH c.estado AS e
               WHERE servi.cuentaCorriente = ?1
                 AND suc.codigoSucursal = ?2
               ORDER BY  c.fechaEmision DESC
                      """, countQuery = """
                                        SELECT COUNT(c) FROM Comprobante c
                                        JOIN c.puntoExpedicion AS pe
                                        JOIN pe.sucursal AS suc
                                        JOIN c.servicio AS servi
                                        WHERE servi.cuentaCorriente = ?1
                                          AND suc.codigoSucursal = ?2
                                        """)
     Page<Comprobante> getComprobantesCuenta(Pageable paget, String cuentaCorriente, Integer codigoSucursal);
     ...
     @Query(value = """
                 SELECT c.*
                  ...
                  LIMIT 1
               """, nativeQuery = true)
     Optional<Comprobante> getUltimoComprobanteCuentaActivo(String cuentaCorriente, Integer codigoSucursal);
