$(function () {
    ...
    $(document).on('click', '.ver-recorrido', async function () {
        ...
        $('#puntosRecorridoCuerpo').html('<tr><td colspan="8" class="text-center py-4"><span class="spinner-border spinner-border-sm"></span> Cargando…</td></tr>');
    ...
    async function cargarPaginaPuntos() {
        if (recorridoDetalleActual === null) return;
        $('#puntosRecorridoCuerpo').html('<tr><td colspan="8" class="text-center py-4">Cargando página…</td></tr>');
        try {
            ...
            $('#puntosRecorridoCuerpo').html(puntos.length ? puntos.map(punto => '<tr>'
                    ...
                    + '<td>' + escapar(punto.velocidadMetrosSegundo === null ? '—' : punto.velocidadMetrosSegundo + ' m/s') + '</td>'
                    + '<td>' + escapar(punto.rumboGrados === null ? '—' : punto.rumboGrados + '°') + '</td><td><code>' + escapar(punto.idDispositivo || '—') + '</code></td>'
                    ...
                    : '<tr><td colspan="8" class="text-center text-muted py-4">Todavía no se recibieron puntos GPS</td></tr>');
