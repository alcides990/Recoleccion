-- Desde esta versión, servicios.fecha_desde es el primer período pendiente.
-- Los comprobantes históricos no se vuelven a sumar para calcular la deuda.
SET NAMES utf8mb4;
...
BEGIN
    ...
    SELECT GREATEST(
               s.fecha_desde,
               COALESCE((SELECT MAX(c.pago_hasta)
                           FROM comprobantes c
                          WHERE c.cuenta_corriente = s.cuenta_corriente
                            AND c.codigo_estado = 1), s.fecha_desde)
           )
      INTO periodo_inicio
      FROM servicios s
...
BEGIN
    ...
    SET periodo_deuda = TIMESTAMPDIFF(
        MONTH, periodo_inicio, DATE_ADD(fecha_cierre, INTERVAL 5 DAY));
